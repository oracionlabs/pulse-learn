...
export class WorkshopsService {
  ...
  async cloneTemplate(templateId: string, orgId: string, createdBy: string) {
    ...
    const templateObj = template.toObject() as unknown as Record<
      string,
      unknown
    >;
    ...
    const rawSteps = (templateObj.steps as unknown[]).map(
    ...
    const relinkedSteps = newSteps.map((step) => {
      const stepScenario = (step as Record<string, unknown>).scenario as
        | { choices: { nextStepId?: string }[] }
        | undefined;
          ...
          scenario: {
            ...
            choices: stepScenario.choices.map((c) => ({
              ...
              nextStepId: c.nextStepId
                ? (idMap[c.nextStepId] ?? c.nextStepId)
                : undefined,
            })),
          },
        };
  ...
  async reorderSteps(
    ...
    workshop.steps = orderedStepIds.map((id, i) => {
