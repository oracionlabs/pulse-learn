...
export class SessionsService {
  ...
  async start(
    ...
    if (!workshop.isPublished)
      throw new BadRequestException('Workshop is not published');
  ...
  async respond(
    ...
    if (dto.stepType === 'quiz' && step.quiz) {
      ...
      const scenario = step.scenario as {
      ...
      points =
        text && text.length >= reflection.minLength
          ? ((step.points as number) ?? 10)
          : 0;
    }
    ...
    const newTimeSpent =
      (session.timeSpentSeconds ?? 0) + (dto.timeSpentSeconds ?? 0);
    ...
    await this.sessionModel.updateOne(
      { _id: sessionId },
      {
        ...
        $set: {
          ...
          scorePercent:
            session.maxScore > 0
              ? Math.round((newScore / session.maxScore) * 100)
              : 0,
          lastActivityAt: new Date(),
  ...
  async getMyHistory(userId: string) {
    return this.sessionModel
      ...
      .populate(
        'workshopId',
        'title vertical difficulty estimatedMinutes totalPoints',
      )
