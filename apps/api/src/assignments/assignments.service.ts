...
export class AssignmentsService {
  ...
  async findAll(
    orgId: string,
    query: { status?: string; workshopId?: string },
  ) {
  ...
  async findMyAssignments(userId: string, orgId: string) {
    return this.assignmentModel
        ...
        $or: [
          {
            ...
            'assignedTo.id': new Types.ObjectId(userId),
          },
          { 'assignedTo.type': 'org' },
        ],
      ...
      .populate(
        'workshopId',
        'title vertical difficulty estimatedMinutes totalPoints isPublished',
      )
    ...
    return this.assignmentModel.create({
      ...
      assignedTo: {
        ...
        id: dto.assignedTo.id
          ? new Types.ObjectId(dto.assignedTo.id)
          : undefined,
      },
  ...
  async update(
    orgId: string,
    assignmentId: string,
    dto: Partial<AssignmentDocument>,
  ) {
  ...
  async detectOverdue() {
    ...
    if (result.modifiedCount > 0) {
      console.log(
        `[Assignments] Marked ${result.modifiedCount} assignments as overdue`,
      );
    }
  }
}
