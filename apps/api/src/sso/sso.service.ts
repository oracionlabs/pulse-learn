...
const MOCK_GOOGLE_USERS: ProviderUser[] = [
  { email: 'alice.wonder@acme.com', name: 'Alice Wonder', department: 'Engineering', externalId: 'g_001' },
  { email: 'bob.builder@acme.com', name: 'Bob Builder', department: 'Product', externalId: 'g_002' },
  { email: 'carol.danvers@acme.com', name: 'Carol Danvers', department: 'Design', externalId: 'g_003' },
]

const MOCK_MICROSOFT_USERS: ProviderUser[] = [
  { email: 'dave.rogers@acme.com', name: 'Dave Rogers', department: 'Sales', externalId: 'ms_001' },
  { email: 'eve.summers@acme.com', name: 'Eve Summers', department: 'HR', externalId: 'ms_002' },
]
...
export class SsoService {
  ...
  private async fetchProviderUsers(provider: 'google' | 'microsoft'): Promise<ProviderUser[]> {
  ...
  async sync(orgId: string, provider: 'google' | 'microsoft') {
    ...
    const existing = await this.userModel.find({ orgId: orgObjectId, ssoProvider: provider })
      ...
      if (existingUser) {
        ...
        if (changed) {
          await this.userModel.updateOne(
            ...
            { name: pu.name, email: pu.email.toLowerCase(), department: pu.department },
          )
    ...
    for (const user of existing) {
      if (!incomingExtIds.has(user.ssoId ?? '') && user.status !== 'deactivated') {
        await this.userModel.updateOne({ _id: user._id }, { status: 'deactivated' })
        deactivated.push(user.email)
      }
    }

    return { provider, added, updated, deactivated, total: providerUsers.length }
  }
}
